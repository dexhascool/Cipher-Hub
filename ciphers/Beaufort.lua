...
local function beaufortCipher(text, keyword)
    local result = {}
    ...
    for char in text:gmatch(".") do
        ...
        if char:match("%a") then
            ...
            local charVal = string.byte(char) - base
            local encodedVal = (shift - charVal + 26) % 26
            table.insert(result, string.char(encodedVal + base))
...
local uiElements = 4
local frame = baseUi.createUi("Beaufort Cipher", uiElements)
...
encodeButton.MouseButton1Click:Connect(function()
    ...
    local encodedText = beaufortCipher(inputText, keyword)
