...
function Blowfish.new(key)
    local self = setmetatable({}, Blowfish)

    self.P = deepCopy(P_INIT)
    ...
    for i = 1, 18 do
        local data = 0
        for k = 1, 4 do
            data = bit32.lshift(data, 8) + keyBytes[j]
            j = j + 1
            if j > keyLen then j = 1 end
        end
        self.P[i] = bit32.bxor(self.P[i], data)
    end

    local L, R = 0, 0
    for i = 1, 18, 2 do
        L, R = self:EncryptBlock(L, R)
        self.P[i] = L
        self.P[i+1] = R
    end
...
function Blowfish:EncryptBlock(L, R)
    for i = 1, 16 do
        ...
        R = bit32.bxor(R, self:F(L))
        L, R = R, L  -- Swap
    end
    L, R = R, L  -- Undo final swap
    R = bit32.bxor(R, self.P[17])
    L = bit32.bxor(L, self.P[18])
    return L, R
end

function Blowfish:EncryptStringBlock(block)
    local L, R = 0, 0
    for i = 1, 4 do
    ...
    L, R = self:EncryptBlock(L, R)
    return toBytes(L) .. toBytes(R)
end

function Blowfish:EncryptString(plaintext)
    plaintext = pad(plaintext)
    local ciphertext = {}
    for i = 1, #plaintext, 8 do
        local block = string.sub(plaintext, i, i + 7)
        table.insert(ciphertext, self:EncryptStringBlock(block))
    end
